(function(f, m) {
	...
	function x(a, h) {
		...
		var e = h.m || h.message || "";
		e = a.m && a.m.message ? e + a.m.message: a.m && a.m.target && a.m.target.tagName ? e + ("Error handler invoked by " + a.m.target.tagName + " tag") : a.m ? e + a.m: a.message ? e + a.message: e + "Unknown error";
		e = {
			m: e,
			name: a.name,
			type: a.type,
		...
		if (!b) {
			e.pageURL = h.pageURL || "" + (window.location ? window.location.href: "") || "missing";
			e.f = a.f || a.sourceURL || a.fileName || a.filename || a.m && a.m.target && a.m.target.src;
			e.l = a.l || a.line || a.lineno || a.lineNumber;
			e.c = a.c ? "" + a.c: a.c;
			e.s = [];
			e.t = f.ue.d();
			if ((b = a.stack || (a.err ? a.err.stack: "")) && b.split) for (e.csm += " stack", k = b.split("\n"); l < k.length && e.s.length < F;)(b = k[l++]) && e.s.push(y(b));
			else {
				e.csm += " callee";
				var m = z(a.args || arguments, "callee");
				for (k = l = 0; m && l < F;) {
					var w = G;
					m.skipTrace || (b = m.toString()) && b.substr && (w = 0 === k ? 4 * G: w, w = 1 == k ? 2 * G: w, e.s.push(b.substr(0, w)), k++);
					m = z(m, "caller");
					l++
				}
			}
	...
	function B(a, h) {
		if (a) {
			for (var b = x(a, h), e = (window.ue_err ? window.ue_err.errorHandlers: null) || [], k = 0; k < e.length; k++)"function" == typeof e[k].handler && e[k].handler(b);
			f.ue.log(b, h.channel || K, {
				nb: 1
			});
			...
			try {
				if (!a.fromOnError) {
					...
					if (l) {
						if (q && q.stringify) try {
							e += q.stringify(b)
						} catch(w) {
							e += "no info provided; converting to string failed"
						} else e += b.m;
						"function" === typeof l.error ? l.error(e, b) : "function" === typeof l.log && l.log(e, b)
					}
				}
			} catch(w) {}
		}
...
})(ue_csm, window); (function(f, m) {
	function y(r) {
		for (var a = {},
		c, d, n = 0; n < r.length; n++) d = r[n],
		d.r = d.r || b.rid,
		d.s = d.s || f.ue_sid,
		c = d.r + d.s + d.m,
	...
	function C(a, t, c) {
		...
		b._lpn[t] = (b._lpn[t] || 0) + 1, c.b ? x(y([a]), I, E) : c.nb ? x(y([a]), I, J, E) : c.img || R[t] ? x(y([a]), E) : c.ff ? (k.push(a), z()) : c.n ? (k.push(a), 0 === Q ? z() : p || (p = m.setTimeout(z, Q))) : (k.push(a), P || (P = m.setTimeout(z, S))))
	}
	function K(a, b, c) {
		M++;
		M == F && C({
		...
		m.ue_err_chan || "jserr"); (M < F || c && c.il) && C(a, b, c)
	}
	function A() {
		if (!w) {
	...
	function L(b) {
		...
		t.csmcount.value += a(t).length, b.reqs.push(t), t = a(b));
		return t
	}
	var D = {}; (function() {
		function a(a) {
		...
		function c(a, d) {
			...
			case "object":
				...
				if (k && "object" === typeof k) for (d = k.length, a = 0; a < d; a += 1)"string" === typeof k[a] && (e = k[a], (h = c(e, g)) && r.push(b(e) + (n ? ": ": ":") + h));
				else for (e in g) Object.prototype.hasOwnProperty.call(g, e) && (h = c(e, g)) && r.push(b(e) + (n ? ": ": ":") + h);
				h = 0 === r.length ? "{}": n ? "{\n" + n + r.join(",\n" + n) + "\n" + t + "}": "{" + r.join(",") + "}";
				n = t;
				return h
			}
		...
		k;
		"function" !== typeof D.stringify && (D.stringify = function(a, b, d) {
			var g;
			f = n = "";
	...
	h = H.stringify,
	b = f.ue || {};
	H = f.uet || v; (f.uet || v)("bb", "ue_frst_v2", {
		wb: 1
	});
	...
	J = function() {
		function a() {
			...
			if (m.ActiveXObject) {
				for (var b = 0; b < d.length && !a; b++) try {
					a = new ActiveXObject(d[b]),
					d = [d[b]]
				} catch(T) {}
				return a
			}
	...
	E = function() {
		return {
			send: function(k) {
				for (var r in k) if (k.hasOwnProperty(r)) {
					for (var c = k[r], d = c, n = {},
					g, l = 0; l < d.length; l++) g = d[l].c,
					n[g] || (n[g] = []),
					n[g].push(d[l]);
					c = c[0] || {};
					d = c.sn || f.ue_sn;
					c = e + (c.m || f.ue_mid) + ":" + (c.s || f.ue_sid) + ":" + (c.r || b.rid) + (d ? ":" + d: "");
					d = [];
					g = c;
					l = [];
					var m = void 0;
					for (m in n) if (n.hasOwnProperty(m)) for (var u = 0; u < n[m].length; u++) {
						var p = n[m][u],
					...
					for (p = 0; p < n.length;) {
						u = n[p];
						l != u.c ? (g += m + u.c + "\x3d", m = "\x26", l = u.c) : g += ",";
						q = g;
						var w = u.d + ":",
						v = u;
						g = (v.l.match(".{1," + (N - g.length) + "}[^%]{0,2}") || [])[0] || "";
						...
						m = "$",
						l = 0;
						else if (p++, 1 != u.p) for (g += ":" + u.p + "_" + u.p, q = 0; q < u.p - 1; q++) d[d.length - q - 1] += u.p
					}
					d.push(g);
					c = d;
	...
	b._lpn = b._lpn || {};
	try {
		m.amznJQ && m.amznJQ.declareAvailable && m.amznJQ.declareAvailable("forester-client"),
